...
<section class="configurator-app">
  <div>
    ...
    <page-intro :title="'Unternehmen + Gastgewerbe'">
    </page-intro>
...
export default {
  ...
  mounted() {
    this.axios.get("/api/data/get/corporate").then(response => {
  ...
  methods: {
    ...
    swapProducts() {
      ...
      if (idxGsm > -1) {
        prod = prod.filter(x => x.communication_gsm == selection[idxGsm].value);
      }

      if (idxEthernet > -1) {
        prod = prod.filter(x => x.communication_ethernet == selection[idxEthernet].value);
      }
