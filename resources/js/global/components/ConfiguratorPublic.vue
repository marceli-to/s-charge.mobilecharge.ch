...
<section class="configurator-app">
  <div>
    ...
    <page-intro :title="'Öffentliche Stationen'"></page-intro>

    <article>

      <!-- Ladestation -->
      <div class="configurator-option" data-row="ladestation" data-require="charge,power,communication,mid">
        ...
        <div class="configurator-option__content">
          <div class="option-elements">
            ...
            <station-performance :variations="true"></station-performance>
            <station-communication-multi></station-communication-multi>
            <station-setup></station-setup>
            <station-mid></station-mid>
            <station-vandalism></station-vandalism>
          </div>
        </div>
      </div>
      <!-- // Ladestation -->
      ...
      <!-- Installation -->
      <div class="configurator-option" data-row="installation" data-require="installation,installation_cable">
        ...
        <div class="configurator-option__content">
          <div class="option-elements">
            ...
            <installation-cable></installation-cable>
          </div>
        </div>
      </div>
      <!-- // Installation -->
...
import StationCommunicationMulti from "@/global/components/configurator/partials/element/StationCommunicationMulti.vue";
import StationMid from "@/global/components/configurator/partials/element/StationMid.vue";
import StationVandalism from "@/global/components/configurator/partials/element/StationVandalism.vue";
import StationSetup from "@/global/components/configurator/partials/element/StationSetup.vue";
...
import InstallationCable from "@/global/components/configurator/partials/element/InstallationCable.vue";
import InstallationSingleLine from "@/global/components/configurator/partials/element/InstallationSingleLine.vue";
...
export default {
  ...
  mounted() {
    this.axios.get("/api/data/get/public").then(response => {
  ...
  methods: {
    ...
    swapProducts() {
      ...
      let idxInstallationCable = selection.findIndex(x => x.prop == 'installation_cable');
      let idxMid               = selection.findIndex(x => x.prop == 'mid');
      let idxGsm               = selection.findIndex(x => x.prop == 'communication_gsm');
      let idxEthernet          = selection.findIndex(x => x.prop == 'communication_ethernet');
      let idxWifi              = selection.findIndex(x => x.prop == 'communication_wifi');
      let idxVandalism         = selection.findIndex(x => x.prop == 'vandalism');
      let idxPower11           = selection.findIndex(x => x.prop == 'power_11');
      let idxPower22           = selection.findIndex(x => x.prop == 'power_22');

      let ids = [];
      let prod = this.products;

      if (idxInstallationCable > -1) {
        prod = prod.filter(x => x.installation_cable == selection[idxInstallationCable].value || x.installation_cable == -1);
      }

      if (idxChargeCable > -1) {
        prod = prod.filter(x => x.charge_cable == selection[idxChargeCable].value || x.charge_cable == -1);
      }

      if (idxChargePlug > -1) {
        prod = prod.filter(x => x.charge_plug == selection[idxChargePlug].value || x.charge_plug == -1);
      }

      if (idxMid > -1) {
        prod = prod.filter(x => x.mid == selection[idxMid].value || x.mid == -1);
      }

      if (idxGsm > -1) {
        prod = prod.filter(x => x.communication_gsm == selection[idxGsm].value || x.communication_gsm == -1);
      }

      if (idxEthernet > -1) {
        prod = prod.filter(x => x.communication_ethernet == selection[idxEthernet].value || x.communication_ethernet == -1);
      }

      if (idxWifi > -1) {
        prod = prod.filter(x => x.communication_wifi == selection[idxWifi].value || x.communication_wifi == -1);
      }

      if (idxVandalism > -1) {
        prod = prod.filter(x => x.vandalism == selection[idxVandalism].value || x.vandalism == -1);
      }

      if (idxPower11 > -1) {
        prod = prod.filter(x => x.power_11 == selection[idxPower11].value || x.power_11 == -1);
      }

      if (idxPower22 > -1) {
        prod = prod.filter(x => x.power_22 == selection[idxPower22].value || x.power_22 == -1);
      }
